...
const createProcess = (name, firstRes, secondRes) => {
  let state = "start"; // 상태: start → holdFirst → done

  const fn = async () => {
    if (state === "start") {
      if (firstRes.own === "") {
        firstRes.own = name;
        console.log(`[${name}] ${firstRes.name} 점유`);
        await randomSleep(name); // 작업 수행
        console.log(`[${name}] ${secondRes.name} 요청`);
        state = "holdFirst";
        processQueue.add(fn); // 첫 리소스를 점유한 상태로 다시 큐에 등록
      } else {
        console.log(`[${name}] ${firstRes.name} 대기`);
        processQueue.add(fn); // 재도전 위해 다시 큐에 등록
      }
    } else if (state === "holdFirst") {
      if (secondRes.own === "") {
        secondRes.own = name;
        console.log(`[${name}] ${secondRes.name} 점유`);
        await randomSleep(name); // 두 리소스를 가진 상태에서 작업 수행
        console.log(`[${name}] 작업 완료, 자원 반납`);
        firstRes.own = "";
        secondRes.own = "";
        state = "done"; // 종료
      } else {
        console.log(`[${name}] ${secondRes.name} 대기`);
        processQueue.add(fn); // 재도전 위해 다시 큐에 등록
      }
    } else if (state === "done") {
